...
\begin{document}
	...
	\begin{frame}{Séparateur}
		...
		\begin{itemize}
			...
			$$ \eta(x) = p(y = 1 \vert x) = 1 - p(y = 0 \vert x)$$
			\item[--] Cela revient à modéliser aussi:
			\begin{align*}
				f: x \mapsto \mathbb{E}_Y(Y \vert X=x) &= \sum_{y=-1, +1} Y.p(Y\vert X=x)\\
				&= (+1) \times p(1\vert x) + (-1) \times p(-1\vert x)
			\end{align*}
		...
		\begin{figure}[H]
			\ffigbox[\FBwidth]
			{
				\begin{subfloatrow}[2]
					...
					{
						\caption{exemples de séparateurs linéaire possible.}\label{fig::lin_sep}
					}
				\end{subfloatrow}
			}
			{
