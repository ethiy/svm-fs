...
\begin{document}
	...
	\begin{frame}{Rappel: Classification supervisée}
		\begin{figure}[H]
			\begin{center}
				\includestandalone[height=.4\textheight]{scatter_gender_dataset}
	...
	\begin{frame}{Supervisé vs Non-Supervisée}
		\begin{figure}[H]
			\begin{center}
				\includestandalone[height=.4\textheight]{scatter_circles}
	...
	\begin{frame}{Discriminatif vs Génératif}
		On suppose que ${\big((X^i, Y^i)\big)}_{i=1,\dots,n}$ sont des réalisations i.i.d de la loi $\mathbb{P}(X=x, Y=y) = p(x,y)$.
		\begin{itemize}
			\only<1-5>{
	...
	\subsection{Un peu de théorie}
		...
		\begin{frame}{Séparateur: illustration}
			\begin{figure}[H]
				\includestandalone[width=.5\textwidth]{scatter_separators}
	...
	\subsection[linear]{SVM linéaire}
		...
		\begin{frame}[plain]{SVM\@: maximiser la marge}
			\begin{figure}[H]
				\includestandalone[width=.75\textwidth]{svm_separable}
			...
			\begin{itemize}
				\only<1-8>{
					...
					\begin{itemize}
						...
						\item<6-> $1 < \xi_i \Rightarrow$ Point de l'autre côté de la ligne séparatrice mais dans la marge: donc mal classé et pas très sûr;
						\item<7-> $2 < \xi_i \Rightarrow$ Point non séparable, il dépasse l'autre limite de marge: sûrement mal classé;
					\end{itemize}
	...
	\begin{frame}{Changement d'espace}
		\begin{figure}[H]
			\begin{center}
				\includestandalone[width=.6\textwidth]{scatter_circles}
