	...
	\begin{frame}{Discriminatif vs Génératif}
		...
		\begin{itemize}
			...
			\only<5-6>{
				...
				\begin{itemize}
					\item <6-> Exemple: On apprend un modèle d'arbres aléatoires.
				\end{itemize}
			}
		\end{itemize}
	\end{frame}
			...
			\begin{figure}[H]
				\ffigbox[\FBwidth]
				{
					\begin{subfloatrow}[2]
						...
						{
							\caption{\tiny Echantillons à séparer.}
						}
						\ffigbox[\FBwidth]
						{
						...
						{
							\caption{\tiny Séparateurs possibles.}
						}
					\end{subfloatrow}
				}
				{
	...
	\begin{frame}{Optimization Quadratique! Peut-on faire mieux?}
		\begin{itemize}
			...
			\begin{equation}
				...
				\end{aligned}
			\end{equation}
            \item<3->  Ce dernier est un problème d'optimisation \textit{linéaire} qu'on sait résoudre plus facilement.
		\end{itemize}
	\end{frame}
